...
def create_pdf(res, raw_data):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", 'B', 16)
    pdf.cell(200, 10, txt="HeartGuard AI Diagnostic Report", ln=True, align='C')
    pdf.set_font("Arial", size=12)
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Risk Probability: {res['prob']*100:.1f}%", ln=True)
    pdf.cell(200, 10, txt=f"Status: {'HIGH' if res['pred']==1 else 'LOW'} RISK", ln=True)
    pdf.cell(200, 10, txt=f"Patient BMI: {res['bmi']:.1f}", ln=True)
    pdf.ln(5)
    pdf.multi_cell(0, 10, txt="Disclaimer: This report is generated by AI for informational purposes only. Consult a doctor for medical diagnosis.")
    return pdf.output(dest='S').encode('latin-1')

model, scaler = load_assets()
# Load your dataset for the Correlation Matrix
data = pd.read_csv("HeartD.csv")
data = data.loc[:, ~data.columns.str.contains("^Unnamed")]
...
if st.session_state.screen == 'input':
    ...
    with st.container():
        ...
        with col1:
            ...
            height = st.slider("Height (cm)", 140, 200, 165)
            weight = st.slider("Weight (kg)", 40, 150, 70)
            chol = st.select_slider("Cholesterol", [1, 2, 3], format_func=lambda x: ["Normal", "High", "Critical"][x-1])
        with col2:
            st.markdown("### 💓 Vitals")
            sbp = st.number_input("Systolic BP", 80, 200, 120)
            dbp = st.number_input("Diastolic BP", 50, 130, 80)
            gluc = st.select_slider("Glucose Level", [1, 2, 3], format_func=lambda x: ["Normal", "High", "Critical"][x-1])
        ...
        if st.button("RUN DIAGNOSTIC ANALYSIS ⚡"):
            bmi = weight / ((height / 100) ** 2)
            input_df = pd.DataFrame([[age, gender, height, weight, sbp, dbp, chol, gluc, int(sm), int(al), int(ac), bmi]], 
                                   columns=scaler.feature_names_in_)
            scaled = scaler.transform(input_df)
            st.session_state.results = {'prob': model.predict_proba(scaled)[0][1], 'pred': model.predict(scaled)[0], 'bmi': bmi, 'raw': input_df}
            st.session_state.screen = 'result'
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

# -------------------------------
# SCREEN 2: Insights, Suggestions & Export
# -------------------------------
else:
    ...
    with col_left:
        fig_gauge = go.Figure(go.Indicator(
            mode = "gauge+number", value = res['prob']*100,
            number = {'suffix': "%", 'font': {'size': 50}},
            title = {'text': "Calculated Risk Probability"},
            gauge = {'axis': {'range': [0, 100]}, 'bar': {'color': "#4facfe"},
                     'steps': [{'range': [0, 30], 'color': "#2ed573"}, {'range': [70, 100], 'color': "#ff4757"}]}))
        st.plotly_chart(fig_gauge, use_container_width=True)

    with col_right:
        st.markdown("### 🧬 Vitals vs. Dataset Average")
        
        avg_vitals = data.mean()
        categories = ['Age', 'Systolic BP', 'Diastolic BP', 'BMI']
        fig_radar = go.Figure()
        fig_radar.add_trace(go.Scatterpolar(r=[raw['age'], raw['systolic_bp'], raw['diastolic_bp'], res['bmi']], theta=categories, fill='toself', name='Patient'))
        fig_radar.add_trace(go.Scatterpolar(r=[avg_vitals['age'], avg_vitals['ap_hi'], avg_vitals['ap_lo'], 25], theta=categories, fill='toself', name='Average'))
        fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 200])), paper_bgcolor='rgba(0,0,0,0)', font=dict(color="white"))
    ...
    with c1:
        st.markdown(f'<div class="metric-box"><h4>Model Accuracy</h4><h1 style="color:#4facfe">72%</h1></div>', unsafe_allow_html=True)
        st.write("")
    ...
    with c2:
        ...
        if res['prob'] > 0.5:
            st.markdown('<div class="suggestion-card"><b>🚨 Immediate Action:</b> High cardiovascular stress detected. Schedule a clinical check-up.</div>', unsafe_allow_html=True)
        if raw['systolic_bp'] > 130:
            st.markdown('<div class="suggestion-card"><b>📉 BP Management:</b> Reduce daily sodium intake and practice mindfulness to lower arterial pressure.</div>', unsafe_allow_html=True)
        if raw['active'] == 0:
            st.markdown('<div class="suggestion-card"><b>🏃 Physical Activity:</b> Start with 20-min daily walks to significantly improve heart efficiency.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="suggestion-card" style="border-left-color: #2ed573"><b>✅ Keep Moving:</b> Your activity levels are effectively protecting your cardiovascular system.</div>', unsafe_allow_html=True)
        
        # EXPORT PDF BUTTON
        pdf_data = create_pdf(res, raw)
        st.download_button(label="📥 Download Diagnostic PDF", data=pdf_data, file_name="Heart_Analysis.pdf", mime="application/pdf")
